...
def model_v_random(model, num_games=100, game_mode="full", t=1, num_players=2, transfer_learning=None):
    env = BarEnv(game_mode=game_mode, self_play=False, t=t, num_players=num_players, transfer_learning=transfer_learning)
    wins = 0
    losses = 0
    draws = 0
...
if __name__ == "__main__":
    ...
    # model = DQN.load("./models/dqn/1tp2transferBasic-Full_lspeed")
    # model.set_env(env)
    ...
    selfplay_callback = SelfPlayCallback(
        ...
        stats_file="model_stats/1tp2transferBasic-Full_dqn",
        stats_file_mode="a",
        transfer_learning=None,
    )
    model.learn(total_timesteps=2_250_000, callback=selfplay_callback)

    model.save("models/dqn/1tp2transferFull_lspeed")

    # model = DQN.load("models/dqn/t1p2basic")
    wins, losses, draws = model_v_random(model, num_games=1000, game_mode="basic", t=1, num_players=2)
